...
async function runSearch(logSearch = true) {
  ...
  try {
    ...
    if (logSearch && $("searchInput").value.trim()) api("/api/searches", { method: "POST", body: JSON.stringify({ actor: "Pilot Admin", query: $("searchInput").value.trim() }) }).catch(() => {});
  } catch (error) {
...
async function openCandidate(candidateId, focus = "profile") {
  ...
  try {
    if (focus !== "history") await api(`/api/candidates/${encodeURIComponent(candidateId)}/view`, { method: "POST", body: JSON.stringify({ actor: "Pilot Admin" }) });
    ...
    $("drawerContent").innerHTML = `
      ...
      <section class="drawer-section hidden" id="resumeSection"><h3>Fictional resume preview</h3><div class="resume-preview">${escapeHtml(candidate.resume_summary)}</div></section>
...
async function showResumePreview(candidateId) {
  ...
  try {
    await api(`/api/candidates/${encodeURIComponent(candidateId)}/resume-open`, { method: "POST", body: JSON.stringify({ actor: "Pilot Admin" }) });
    toast("Fictional resume preview recorded");
    loadMeta();
...
function openCallModal(candidateId) {
  ...
  if (!candidate) return;
  state.selected = candidate;
  $("callCandidateName").textContent = `${candidate.name} · fictional pilot profile`;
...
async function saveCall(event) {
  ...
  try {
    await api(`/api/candidates/${encodeURIComponent(state.selected.id)}/calls`, { method: "POST", body: JSON.stringify({ recruiter: "Pilot Admin", role: $("callRole").value, outcome: $("callOutcome").value, note: $("callNote").value }) });
    closeCallModal();
...
function renderMeta() {
  ...
  $("sourceGrid").innerHTML = (state.meta.sources || []).map((source) => `<article class="source-card"><header><h3>${escapeHtml(source.label)}</h3><span>● ${escapeHtml(source.status)}</span></header><p>${escapeHtml(source.kind)} · fictional pilot connector</p><div class="source-stats"><div><b>${source.total_rows}</b><small>source rows</small></div><div><b>${source.synced_rows}</b><small>synced</small></div><div><b>${source.failed_rows}</b><small>failed</small></div><div><b>${source.duplicate_rows}</b><small>duplicates</small></div></div><footer><span>${source.connected ? "Connected" : "Disconnected"}</span><span>Last sync ${escapeHtml(formatDate(source.last_sync, true))}</span></footer></article>`).join("");
  $("jobList").innerHTML = (state.meta.jobs || []).map((job) => { const percent = job.total_rows ? Math.round(job.processed_rows / job.total_rows * 100) : 100; return `<div class="job-row"><div><b>${escapeHtml(job.source_label)}</b><small>${escapeHtml(job.stage)} · ${escapeHtml(job.message)}</small></div><div class="progress"><i style="width:${percent}%"></i></div><span class="job-status">${escapeHtml(job.status)} · ${percent}%</span></div>`; }).join("") || "<div class='job-row'>No pilot jobs yet.</div>";
...
Promise.all([loadMeta(), runSearch(false)]);
